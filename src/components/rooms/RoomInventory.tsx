...
export default function RoomInventoryPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchRoomAndInventory = async () => {
      ...
      try {
        ...
        // Fetch inventory for this room - UPDATED to use the new endpoint structure
        const inventoryResponse = await fetch(`https://api.blexi.co/api/v1/inventory?assignable_type=App\\Modules\\Room\\Models\\Room&assignable_id=${params.id}&per_page=100`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
            'Accept': 'application/json',
  ...
  const fetchAvailableInventory = async () => {
    ...
    try {
      // Updated to use the new endpoint structure with status parameter
  ...
  const handleAssignInventory = async () => {
    ...
    try {
      const response = await fetch(`https://api.blexi.co/api/v1/inventory/${selectedItem}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleUnassignInventory = async () => {
    ...
    try {
      const response = await fetch(`https://api.blexi.co/api/v1/inventory/${itemToUnassign}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
